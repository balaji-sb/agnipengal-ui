        ...
        <div className='flex items-center gap-4'>
          ...
          <div>
            <h1 className='text-xl font-bold text-gray-900 line-clamp-1 flex items-center gap-2'>
              {ticket.subject}
              <span
                className={`text-xs px-2 py-0.5 rounded border border-gray-200 font-normal
                                ${
      ...
      <div className='flex-1 bg-gray-50 rounded-2xl border border-gray-200 overflow-hidden flex flex-col shadow-sm'>
          ...
          {ticket.messages.map((msg: any, index: number) => {
            ...
            return (
              <div key={index} className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                <div
                  className={`flex max-w-[80%] ${isAdmin ? 'flex-row-reverse' : 'flex-row'} gap-3`}
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isAdmin ? 'bg-pink-100 text-pink-600' : 'bg-gray-200 text-gray-600'}`}
                  >
                  ...
                  >
                        ...
                        <a
                          ...
                          className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition ${
                            isAdmin
                              ...
                              : 'bg-gray-50 text-pink-600 hover:bg-gray-100'
                          }`}
                        >
                    ...
                    <div
                      className={`text-[10px] mt-2 opacity-70 flex items-center gap-1 ${isAdmin ? 'justify-end' : 'justify-start'}`}
                    >
        ...
        <div className='p-4 bg-white border-t border-gray-200'>
          <div className='relative flex gap-3 items-end'>
            <label
              className={`p-3 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50 transition flex-shrink-0 ${uploading ? 'opacity-50' : ''}`}
            >
              <input
                type='file'
                className='hidden'
              ...
              {uploading ? (
                ...
                <Paperclip
                  className={`w-5 h-5 ${attachment ? 'text-pink-600' : 'text-gray-500'}`}
                />
              )}
            </label>
            ...
            <div className='flex-1 relative'>
              {attachment && (
                <div className='absolute -top-10 left-0 bg-pink-50 text-pink-700 text-xs px-2 py-1 rounded-lg border border-pink-100 flex items-center gap-2'>
                  ...
                  Attached File
                  <button onClick={() => setAttachment('')} className='hover:text-pink-900 ml-1'>
                    ×
                  </button>
                </div>
              )}
            ...
            <button
              ...
              className='p-3 bg-orange-600 text-white rounded-xl hover:bg-orange-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0 mb-0.5'
            >
              {submitting ? (
