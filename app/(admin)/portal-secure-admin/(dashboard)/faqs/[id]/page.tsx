        ...
        if (!isNew && id) {
            const fetchFAQ = async () => {
                try {
                    ...
                    router.push('/portal-secure-admin/faqs');
                } finally {
                    setLoading(false);
                }
            };
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div className="flex items-center pt-6">
                            <label className="flex items-center gap-3 cursor-pointer">
                                <div className="relative">
                                    ...
                                    />
                                    <div className={`block w-12 h-7 rounded-full transition ${formData.isActive ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                                    <div className={`dot absolute left-1 top-1 bg-white w-5 h-5 rounded-full transition transform ${formData.isActive ? 'translate-x-5' : 'translate-x-0'}`}></div>
                                </div>
                ...
                <div className="bg-gray-50 px-6 py-4 flex justify-end gap-3 border-t border-gray-100">
                    ...
                    <button 
                        type="submit" 
                        disabled={saving}
                        className="px-6 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 transition font-medium shadow-sm hover:shadow flex items-center gap-2 disabled:opacity-70 disabled:cursor-wait"
                    >
                        {saving ? (
                            <>
