...
export default function ProductDetails({ product }: ProductDetailsProps) {
    ...
    const displayPrice = currentVariant ? currentVariant.price : (product.offerPrice > 0 ? product.offerPrice : product.price);
    ...
    const discount = isOffer ? Math.round(((product.price - product.offerPrice) / product.price) * 100) : null;
    ...
    const displayImages = (currentVariant && currentVariant.image) 
                ...
                {availableKeys.length > 0 && (
                    <div className="space-y-4 border-t border-gray-100 pt-6">
                        {availableKeys.map((key) => (
                            <div key={key}>
                                <h3 className="text-sm font-medium text-gray-900 mb-3 uppercase tracking-wide">{key}</h3>
                        ...
                        {!currentVariant && (
                            <p className="text-sm text-red-500">Combination not available</p>
                        )}
                    </div>
                )}
